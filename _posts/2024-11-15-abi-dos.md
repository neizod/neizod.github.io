...
Take a level where you're asked to compute $1+2+3+...+n$. You could simply loop through each integer and add them up, just as the problem suggests. Or, you might derive the mathematical formula $n(n+1)/2$ for a faster solution. Or maybe you observed that the constraint n is relatively small enough to cache all the possible pre-computed answers.
